...
$allowedStatus = array("Signed", "RevokeError");
...
<html>
  ...
  <body>
    ...
    Check and revoke an allready existing Certificate here.<br/>
    Pr&uuml;fe und best&auml;tige das Zur&uuml;ckziehen eines bereits bestehenden Zertifikates hier.
...
if (in_array($extension, $allowedExts))
{
  ...
  else
  {
    ...
    {
      echo "<p><b>Error</b>: Invalid request. CSR/Cert not found.<br/><b>Fehler</b>: Ung&uuml;ltige Anfrage. CSR/Zertifikat nicht gefunden.</p>";
    }
    else
    {
      if (!file_exists($jsonfile))
      {
        echo "<p><b>Error</b>: Invalid request. CSR/Cert-JSON not found.<br/><b>Fehler</b>: Ung&uuml;ltige Anfrage. CSR/Zertifikat-JSON nicht gefunden.</p>";
      }
      else
      {
	// debug only (do not show to the user in normal operation)
        if ($debug) echo "CSR/Cert and JSON file found.<br/>";
        // inspect csr file
	...
	echo "<tr><td>Mail:</td><td>to:" . $mailto . " cc:" . $mailcc . "</td></tr>";
        ...
        if ($approve == "true") 
        {
          ...
          {
            $json["status"] = "RevokeApproved";
	    $json["revokeapprove_message"] = "subject=" . $approver_dn . ", serial=" . $approver_sn;
            $json["revokeapprove_timestamp"] = time();
            file_put_contents($jsonfile, json_encode($json));
            echo "<b>Success</b>: OK - Approved / Best&auml;tigt!";
          }
          else
          {
            echo "<b>Error</b>: Status of request is '" . $json["status"] . "'. Already revoked?<br/><b>Fehler</b>: Status der Anfrage ist '" . $json["status"] . "'. Bereits zur&uuml;ckgezogen?";
          }
        }
        else
        {
          echo "<form action=\"revoke_approve.php?". $name . "\" method=\"post\" enctype=\"multipart/form-data\">";
